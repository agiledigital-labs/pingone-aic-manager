...
Files marked _managed_ (everything under `types/`, the `tsconfig.json`,
`eslint.config.js`, and `tools/`) are regenerated by `aic workspace update`;
everything else is yours. Update also deletes managed files retired in
...
aic workspace update                 # refresh the managed types/config
```
