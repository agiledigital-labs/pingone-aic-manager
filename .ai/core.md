...
| Script sync (pull/push/sync/watch/diff)                                                                                                | `src/scripts/` (one module per `Kind`: `am`, `idm`, `schedule`, `managed_hooks`, `sync_mapping`). Every tenant write goes through `sync.rs`'s `write_checked`, which runs the pre-push syntax gate in `syntax.rs` — do not call `Kind::write` directly. The gate fails **closed**: only a pass, or an `Unsupported` decided from the resource before any request, may write; a check that gives no verdict writes nothing. `write_checked` returns its outcome and prints nothing — presentation is per-surface (`cli.rs` to stderr, the tab's inline strip). The test in `sync.rs` is a **tripwire, not an invariant**: it greps this one module for the single obvious spelling of a direct write, and cannot see a renamed binding, a call split across lines, a raw `am::write` (still `pub`), or any other file — so a new write site needs review, not just a green suite                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `docs/api/04-scripts.md`, `11`, `12`, `13`, `16`               |
