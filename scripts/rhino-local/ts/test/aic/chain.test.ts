...
function mockChain(
  options: { finishImmediately?: boolean; before?: Record<string, unknown> } = {}
): FakeChain {
  ...
  const dump = JSON.stringify({ outcome: "done", before: seen, final: seen });
  const finished = {
    callbacks: [
      {
        type: "HiddenValueCallback",
      ...
      if (args.includes("ctx") && args.includes("list")) {
        return Promise.resolve({
          ...
          stdout: JSON.stringify([
            { current: true, name: "sandbox", theme: "sandbox", base_url: PLACEHOLDER_BASE },
          ]),
          stderr: "",
        });
      }
    ...
    http(req: HttpRequest): Promise<HttpResponse> {
      ...
      if (req.method === "PUT") {
        ...
        if (url.pathname.includes("/trees/")) {
          fake.treesCreated.push(decodeURIComponent(url.pathname.split("/trees/")[1] ?? ""));
        }
        if (typeof body.script === "string" && String(body.name).endsWith("-subject")) {
          fake.subjectSource = Buffer.from(body.script, "base64").toString("utf8");
        }
        if (Array.isArray(body.outcomes) && url.pathname.includes("/ScriptedDecisionNode/")) {
