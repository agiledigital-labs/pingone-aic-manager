// Corpus copy of scripts/rhino-script-tester/fixtures-legacy/legacy-request-multivalue.script.js. Do not stub missing bindings.

// Legacy (evaluatorVersion 1.0) probe: do requestHeaders / requestParameters
// values ever hold MORE THAN ONE element? Send the same header twice and the
// same query parameter twice, then report the element count each binding
// reports back.
//
...
function emit(payload) {
  ...
  if (callbacks.isEmpty()) {
    action = frJava.Action.send(new frJava.HiddenValueCallback("result", json)).build();
  } else {
