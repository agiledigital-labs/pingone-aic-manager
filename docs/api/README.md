...
| [22-token-exchange.md](22-token-exchange.md)                       | **RFC 8693 token exchange** — the capability-token mint. `may_act` on the subject token is the gate and a script stamps it; **the exchange widens scope unless something stops it**; `usePolicyEngineForScope` engages but gets no subject, so the working gate is a next-gen `validate-scope` script, where `identity` is empty and the resource owner comes from `requestProperties.requestParams`. Discovery does not advertise the grant; `audience` is silently ignored |
