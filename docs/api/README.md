...
| [04-scripts.md](04-scripts.md)                                     | All script contexts, base64 body, no `_rev`, `?_action=validate`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| [05-oauth2-oidc.md](05-oauth2-oidc.md)                             | OAuth2 clients (agents), OIDC provider service                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| [06-saml.md](06-saml.md)                                           | Hosted/remote entities, CoT, metadata XML. **CoT membership is stored in two places and REST shows only one** — read that section before touching federation config or diagnosing a rejected assertion                                                                                                                                                                                                                                                                                                                           |
...
| [11-idm-endpoints.md](11-idm-endpoints.md)                         | IDM custom endpoints + scheduled jobs (`config/endpoint/*`, `config/schedule/*`), plaintext `source`, no `_rev`, `script?_action=compile`                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| [21-am-policies.md](21-am-policies.md)                             | **AM policies** — resource types, policy sets (`applications`) and policies, plus `?_action=evaluate`, the PDP call a resource server makes. Create is **asymmetric** (resource types `PUT` with a chosen id; policies and policy sets are `POST ?_action=create` only). `/subjecttypes` + `/conditiontypes` are the machine-readable catalog. **`subject.jwt` is unverified input — AM checks neither signature nor expiry, so the PEP must validate the token before evaluating it.** With a `jwt` subject `AuthenticatedUsers` never matches and `JwtClaim` matches **inside array claims** (including `scope`); `AuthLevel` **500s**. Wildcards measured: `*` crosses `/`, `-*-` does not, a query string is part of the resource, matching is case-insensitive. Implemented by `aic policy` |
| [22-token-exchange.md](22-token-exchange.md)                       | **RFC 8693 token exchange** — the capability-token mint. Read it with `aic oauth exchange list`. `may_act` on the subject token is the gate and a script stamps it; **the exchange widens scope unless something stops it**; `usePolicyEngineForScope` engages but gets no subject, so the working gate is a next-gen `validate-scope` script, where `identity` is empty and the resource owner comes from `requestProperties.requestParams`. Discovery does not advertise the grant; `audience` is silently ignored |
