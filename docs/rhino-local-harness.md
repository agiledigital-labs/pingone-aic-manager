...
The original report also said a key written with `nodeState.putTransient`
arrived in shared state. That was not measured. The result node observed the
key through unified `nodeState.get`; `classifyFinal` supplied the shared
attribution. New and changed unified values are now recorded as unbucketed,
with their possible buckets, instead of being assigned to shared state.
