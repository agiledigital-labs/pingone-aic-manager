...
| IDM authorization rules (`config/access`: list/add/edit/rm/apply)                                                                      | `src/access/` — `spec.rs` holds the input specs, structural validation and the rule/document digests that address a rule; `ops.rs` the in-place `Value` transforms both `cli.rs` and the tab call. The Access tab is **browse-only**: `screen`/`view`/`state` read the whole document and never write, so all editing is still `aic access` | `docs/api/19-config-access.md`                               |
