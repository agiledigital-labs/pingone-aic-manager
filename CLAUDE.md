...
- **Don't `PUT` a managed *record* twice.** A repeat `PUT` to
  ...
  instead. And a converging `PATCH` must **omit** `password`: re-sending the
  one the user already has trips the realm's password history
